...
function doPay(e, done, err) {
    if (e.Amount = window.__TinkoffNormalizeMoney(e.Amount), e.prePay) {
        ...
        if (i.open("GET", o, !1), i.send(null), response = i.responseText, 0 == JSON.parse(response).ErrorCode && JSON.parse(response).Success) {
            ...
            alert("Извините, произошла ошибка при регистрации заказа. Ошибка: " + JSON.parse(response).ErrorCode + " " + JSON.parse(response).Message + " " + JSON.parse(response).Details)
            typeof err === "function" && err();
        }
    }
}

function pay(e, done, err) {
...
TinkoffUrl = {CONTEXT_URL: "https://securepay.tinkoff.ru/"}, TinkoffWidgetConstants = {
    HEIGHT: '800px',
    WIDTH: '1000px',
    MAX_WIDTH: '1200px',
    LEFT: '50%',
